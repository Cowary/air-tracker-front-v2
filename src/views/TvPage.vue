    ...
    <div class="media-list-section">
      ...
      <div v-else class="media-table-container">
        <table class="media-table">
          ...
          <tbody>
            <tr v-for="season in mediaList" :key="season.id">
              <td class="title-cell">{{ season.title }}</td>
              <td>{{ season.originalTitle || '—' }}</td>
              <td>{{ season.number || '—' }}</td>
              <td>{{ season.episodesEnd || 0 }} / {{ season.episodes || '?' }}</td>
              <td>
                <span class="status-badge" :class="getStatusClass(season.status)">
                  {{ getStatusLabel(season.status) }}
                </span>
              </td>
              <td>{{ season.releaseYear || '—' }}</td>
              <td>
                <span v-if="season.score" class="score">{{ season.score }}</span>
                <span v-else>—</span>
              </td>
              <td>
                <div class="actions">
                  <button @click="editMedia(season)" class="btn-icon" title="Редактировать">
                  ...
                  <button @click="deleteMedia(season.id)" class="btn-icon btn-delete" title="Удалить">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="closeForm">
      <div class="modal">
        <div class="modal-header">
          <h2>{{ editingMedia ? 'Редактировать сезон' : 'Добавить сезон' }}</h2>
...
const loadMedia = async () => {
  loading.value = true;
  try {
    const response = await tvApi.getAll(userId.value);
    mediaList.value = response.data;
  } catch (err) {
    console.error('Ошибка загрузки сериалов:', err);
  } finally {
    loading.value = false;
  }
};
...
const searchMedia = async () => {
  ...
  try {
    const response = await tvApi.search(searchQuery.value);
...
const selectFromSearch = async (item) => {
  ...
  try {
    const response = await tvApi.getByServiceId(item.integrationId);
    ...
    if (mediaData && mediaData.media) {
      ...
      formData.value = {
        ...
        episodesEnd: media.episodesEnd || 0,
    ...
    console.error('Ошибка получения данных сериала:', err);
  }
};
...
const saveMedia = async () => {
  try {
    ...
    if (editingMedia.value) {
      await tvApi.update(data, userId.value);
    } else {
      await tvApi.create(data, userId.value);
    }
    ...
    alert('Ошибка сохранения сезона');
  }
};
...
const deleteMedia = async (id) => {
  ...
  try {
    await tvApi.delete(id, userId.value);
    await loadMedia();
  } catch (err) {
