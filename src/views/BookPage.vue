    ...
    <div class="media-list-section">
      ...
      <div v-else class="media-table-container">
        <table class="media-table">
          ...
          <tbody>
            <tr v-for="book in mediaList" :key="book.id">
              ...
              <td>{{ book.endDate || '—' }}</td>
              <td>
                <span v-if="book.score" class="score">{{ book.score }}</span>
                <span v-else>—</span>
              </td>
              <td>
                <div class="actions">
                  <button @click="editMedia(book)" class="btn-icon" title="Редактировать">
                  ...
                  <button @click="deleteMedia(book.id)" class="btn-icon btn-delete" title="Удалить">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="closeForm">
      <div class="modal">
        <div class="modal-header">
          <h2>{{ editingMedia ? 'Редактировать книгу' : 'Добавить книгу' }}</h2>
...
const loadMedia = async () => {
  loading.value = true;
  try {
    const response = await bookApi.getAll(userId.value);
...
const saveMedia = async () => {
  try {
    ...
    if (editingMedia.value) {
      await bookApi.update(data, userId.value);
    } else {
      await bookApi.create(data, userId.value);
    }
...
const deleteMedia = async (id) => {
  ...
  try {
    await bookApi.delete(id, userId.value);
    await loadMedia();
  } catch (err) {
