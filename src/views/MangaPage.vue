    ...
    <div class="media-list-section">
      ...
      <div v-else class="media-table-container">
        <table class="media-table">
          ...
          <tbody>
            <tr v-for="manga in mediaList" :key="manga.id">
              <td class="title-cell">{{ manga.title }}</td>
              <td>{{ manga.originalTitle || '—' }}</td>
              <td>{{ manga.volumesEnd || 0 }} / {{ manga.volumes || '?' }}</td>
              <td>{{ manga.chaptersEnd || 0 }} / {{ manga.chapters || '?' }}</td>
              <td>
                <span class="status-badge" :class="getStatusClass(manga.status)">
                  {{ getStatusLabel(manga.status) }}
                </span>
              </td>
              <td>{{ manga.releaseYear || '—' }}</td>
              <td>
                <span v-if="manga.score" class="score">{{ manga.score }}</span>
                <span v-else>—</span>
              </td>
              <td>
                <div class="actions">
                  <button @click="editMedia(manga)" class="btn-icon" title="Редактировать">
                  ...
                  <button @click="deleteMedia(manga.id)" class="btn-icon btn-delete" title="Удалить">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="closeForm">
      <div class="modal">
        <div class="modal-header">
          <h2>{{ editingMedia ? 'Редактировать мангу' : 'Добавить мангу' }}</h2>
        ...
        <form @submit.prevent="saveMedia" class="media-form">
          ...
          <div class="form-row">
            <div class="form-group">
              <label>Томы (текущие)</label>
              <input v-model.number="formData.volumesEnd" type="number" />
            </div>
            <div class="form-group">
              <label>Всего томов</label>
              <input v-model.number="formData.volumes" type="number" />
            </div>
          </div>

          <div class="form-row">
            <div class="form-group">
              <label>Главы (текущие)</label>
              <input v-model.number="formData.chaptersEnd" type="number" />
            </div>
            <div class="form-group">
              <label>Всего глав</label>
              <input v-model.number="formData.chapters" type="number" />
            </div>
          </div>
...
const loadMedia = async () => {
  loading.value = true;
  try {
    const response = await mangaApi.getAll(userId.value);
...
const searchMedia = async () => {
  ...
  try {
    const response = await mangaApi.search(searchQuery.value);
...
const selectFromSearch = async (item) => {
  ...
  try {
    const response = await mangaApi.getByServiceId(item.integrationId);
    ...
    if (mediaData && mediaData.media) {
      ...
      formData.value = {
        ...
        volumes: media.volumes || formData.value.volumes,
        chapters: media.chapters || formData.value.chapters,
        score: media.score || formData.value.score,
        releaseDate: media.releaseDate || '',
        releaseYear: media.releaseYear || formData.value.releaseYear,
        endDate: media.endDate || '',
        shikiId: media.shikiId || formData.value.shikiId,
        volumesEnd: media.volumesEnd || 0,
        chaptersEnd: media.chaptersEnd || 0
      };
    } else if (mediaData.posterUrl) {
      posterUrl.value = mediaData.posterUrl;
    }
  } catch (err) {
    console.error('Ошибка получения данных манги:', err);
  }
};
...
const saveMedia = async () => {
  try {
    ...
    if (editingMedia.value) {
      await mangaApi.update(data, userId.value);
    } else {
      await mangaApi.create(data, userId.value);
    }
...
const deleteMedia = async (id) => {
  if (!confirm('Вы уверены, что хотите удалить эту мангу?')) return;

  try {
    await mangaApi.delete(id, userId.value);
    await loadMedia();
  } catch (err) {
    console.error('Ошибка удаления:', err);
    alert('Ошибка удаления манги');
  }
};
