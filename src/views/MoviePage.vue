    ...
    <div class="media-list-section">
      ...
      <div v-else class="media-table-container">
        <table class="media-table">
          ...
          <tbody>
            <tr v-for="movie in mediaList" :key="movie.id">
              <td class="title-cell">{{ movie.title }}</td>
              <td>{{ movie.originalTitle || '—' }}</td>
              <td>
                <span class="status-badge" :class="getStatusClass(movie.status)">
              ...
              <td>
                <span v-if="movie.score" class="score">{{ movie.score }}</span>
                <span v-else>—</span>
              </td>
              <td>
                <div class="actions">
                  <button @click="editMedia(movie)" class="btn-icon" title="Редактировать">
                  ...
                  <button @click="deleteMedia(movie.id)" class="btn-icon btn-delete" title="Удалить">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="closeForm">
      <div class="modal">
        <div class="modal-header">
          <h2>{{ editingMedia ? 'Редактировать фильм' : 'Добавить фильм' }}</h2>
        ...
        <form @submit.prevent="saveMedia" class="media-form">
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input v-model.number="formData.releaseYear" type="number" required />
            </div>
          </div>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input v-model.number="formData.score" type="number" min="1" max="10" required />
            </div>
            <div class="form-group">
...
const loadMedia = async () => {
  loading.value = true;
  try {
    const response = await movieApi.getAll(userId.value);
...
const searchMedia = async () => {
  ...
  try {
    const response = await movieApi.search(searchQuery.value);
...
const selectFromSearch = async (item) => {
  ...
  try {
    const response = await movieApi.getByServiceId(item.integrationId);
    ...
    console.error('Ошибка получения данных фильма:', err);
  }
};
...
const saveMedia = async () => {
  try {
    ...
    if (editingMedia.value) {
      await movieApi.update(data, userId.value);
    } else {
      await movieApi.create(data, userId.value);
    }
...
const deleteMedia = async (id) => {
  ...
  try {
    await movieApi.delete(id, userId.value);
    await loadMedia();
  } catch (err) {
